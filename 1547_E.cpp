...
int32_t main()
{
	...
	for (int t = 1; t <= tt; t++){
		...
		for (int i = 0; i < k; i++){
			int tn = vp[i].F;
			int p = vp[i].S , q = vp[i].S, curr = vp[i].S;
			dp[curr] = min(dp[curr] , tn);
			--p , ++q;
			while(p >= 1 || q <= n){
				if (dp[p] < tn + abs(curr - p) and dp[q] < tn + abs(curr - q)) break;
				if (p >= 1){
					dp[p] = min(dp[p] , tn + abs(curr - p));
					--p;	
				}
				if (q <= n){
					dp[q] = min(dp[q] , tn + abs(curr - q));
					++q;
				}
